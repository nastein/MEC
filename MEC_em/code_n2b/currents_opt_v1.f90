...
subroutine current_init(p1_in,p2_in,pp1_in,pp2_in,q_in,w_in,k1_in,k2_in,i_fl_in,total_isospin_in)
    implicit none
...
subroutine det_JaJc_dir(r_cc,r_cl,r_ll,r_t,r_tp)
   ...
   if(pair_isospin.ne.1 .and. pair_isospin.ne.0) then
        ciso1 = 2.0d0*IDeltaADag_EM(t1p,t2p,t1,t2)*IDeltaA_EM(t1,t2,t2p,t1p)
        ciso2 = 2.0d0*IDeltaBDag_EM(t1p,t2p,t1,t2)*IDeltaB_EM(t1,t2,t2p,t1p)
        ciso3 = 2.0d0*IDeltaADag_EM(t1p,t2p,t1,t2)*IDeltaB_EM(t1,t2,t2p,t1p)
   else
        ciso1 = 16.0d0/3.0d0
        ciso2 = -16.0d0/9.0d0
        ciso3 = 16.0d0/9.0d0
...
!!!! what we want to compute is
 ...
 subroutine det_JpiJaJb(r_cc,r_cl,r_ll,r_t,r_tp)
   ...
   if(pair_isospin.ne.1 .and. pair_isospin.ne.0) then
        ciso1 = -2.0d0*Ivz(t1p,t2p,t1,t2)*IDeltaA_EM(t1,t2,t1p,t2p)
        ciso2 = -2.0d0*Ivz(t1p,t2p,t1,t2)*IDeltaB_EM(t1,t2,t1p,t2p)
        !Changing convention
        ciso1 = -ciso1
        ciso2 = -ciso2
   else
...
!   r_av(4,4)=(w/q(4))**2*r_av(1,1)
   ...
   if(pair_isospin.ne.1 .and. pair_isospin.ne.0) then
        ciso = -2.0d0*Ivz(t1p,t2p,t1,t2)*IDeltaA_EM(t1,t2,t2p,t1p)
        !Changing convention
        ciso = -ciso
   else
        ciso = 16.0d0/3.0d0
...
subroutine det_JpiJpi(r_cc,r_cl,r_ll,r_t,r_tp)
   ...
   if(pair_isospin.ne.1 .and. pair_isospin.ne.0) then
        ciso = -2.0d0*Ivz(t1p,t2p,t1,t2)*Ivz(t1,t2,t1p,t2p)
   else
        ciso = 16.0d0
   endif
