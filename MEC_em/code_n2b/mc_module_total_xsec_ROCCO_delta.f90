module mc_module
   ...
   real*8, private, parameter :: pi=acos(-1.0d0),hbarc=197.327053d0,xmp=938.0d0,ppmax=3.0d0*1.e3
...
subroutine mc_init(i_fg_in,pwia,i_fsi_in,irn_in,nev_in,nwlk_in,xpf_in,thetalept_in,xmpi_in,xmd_in,xmn_in,xA_in, &
  ...
  use mathtool
   ...
   dp0=dp0/hbarc**6
!   do i=1,np
!      write(1000,*) pv(i), sum(dp(i,:)*pv(:)**2)*4.0d0*pi*(pv(2)-pv(1))/(2*pi)**3
!   enddo   
!   stop
!   if(i_fg.ne.1) then
!      np=2*np0
...
!   endif
      ...
      do i=1,np
         do j=1,np
          ...
          enddo
      enddo    
      !write(6,*) 'norm tot = ', norm
      !write(6,*) 'norm pp = ',norm1
      !write(6,*)'norm pn = ',norm0
      ...
      enddo  
      !write(6,*) 'norm tot =' , norm
      !write(6,*) 'pp/tot = ',norm1/norm
      !write(6,*)'np/tot =' ,norm0/norm
...
subroutine int_eval(ctpp1,p2,ctp2,phip2,p1,ctp1,phip1,ip1,ip2,w,qval,r_now,nk)
   ...
   real*8, parameter :: fstar=2.13d0
   real*8, parameter :: eps=10.0d0,e_gs=-92.16,e_bg=-64.75
   integer*4 :: ip1,ip2
...
!....at this point we can define pp1_4
  ...
  ! endif
   ...
   !...delta function
   !Noah removed the 40 MeV FSI
   arg=q_4(1)+p1_4(1)+p2_4(1)-pp1_4(1)-pp2_4(1)!+40.0d0
   !arg=q_4(1)+e_gs-e_bg+xmn*2.0d0-pp1_4(1)-pp2_4(1)+40.0d0
   delta_w=fdelta(arg,eps)
!...define pion momenta
   k1_4(:)=pp1_4(:)-p1_4(:)
   !k2_4(:)=pp2_4(:)-p2_4(:)
   k2_4(:)=q_4(:)-k1_4(:)
   !if(k2_4(1).lt.0d0 .or. k2_4(1)**2-sum(k2_4(2:4)**2) .gt. xmpi**2) then
   !  r_now=0.0d0
   !  return
   !endif
   k1e_4(:)=pp2_4(:)-p1_4(:)
   k2e_4(:)= q_4(:)-k1e_4(:)!q_4(:)-k1e_4(:)
