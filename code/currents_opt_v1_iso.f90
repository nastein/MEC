module dirac_matrices
    ...
    complex*16, private, save :: sig(3,2,2),id(2,2),id4(4,4),up(2),down(2),isospinor(2,2),iso(2,2)
...
!
    ...
    enddo
    if(i_fl.eq.1) then
       Pi_k1(:,:)=matmul(gamma_mu(:,:,5),k1_sl(:,:))/(k1(1)**2-sum(k1(2:4)**2)-xmpi**2)!Eq 53
...
! This function computes the currents corresponding to 
! Pion in flight
! Seagul
! Pion pole
subroutine det_Jpi(gep)
   ...
   fpik1=(lpi**2-xmpi**2)/(lpi**2-k1(1)**2+sum(k1(2:4)**2)) !Eq 56
   fpik2=(lpi**2-xmpi**2)/(lpi**2-k2(1)**2+sum(k2(2:4)**2)) !Eq 56
   frho1=0.0d0!1.0d0/(1.0d0-(k1(1)**2-sum(k1(2:4)**2))/xmrho**2) !These are set to zero for some reason 
   ...
   do mu=1,4
      J_pif(:,:,mu)=gep*(k1(mu)-k2(mu))*Pi_k1(:,:)!*fact ! Eq 52
   ...
   enddo
  J_pif=J_pif*fpik1*fpik2*fpinn2/xmpi**2 !Eq 52
...
subroutine det_JaJc_dir(r_cc,r_cl,r_ll,r_t,r_tp)
  ...
  ! Here we are summing over the final state isospins
   ! ti1 and ti2 are the initial states
   ! t1 and t2 are the final states 
   do t1=1,2
    do t2 =1,2
        iso_coeff(1) = iso_coeff(1) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
           &  *IDeltaA(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:)) !AA
        iso_coeff(2) = iso_coeff(2) + IDeltaBDag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaB(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:))!BB
        iso_coeff(3) = iso_coeff(3) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaB(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:))!AB
        iso_coeff(4) = iso_coeff(4) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaC(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:))!AC
        iso_coeff(5) = iso_coeff(5) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaD(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:)) !AD
        iso_coeff(6) = iso_coeff(6) + IDeltaBDag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
           &  *IDeltaC(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:))!BC
        iso_coeff(7) = iso_coeff(7) + IDeltaBDag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaD(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:))!BD
    enddo
   enddo 

   do i=1,7
    write(6,*)'jajac dir iso coeff(',i,') = ', iso_coeff(i)
    enddo
   !Covers pp and nn case for initial states
   !multplied by two fr not looking into jcjc,jdjd,jcjd,etc.
   jaja(:,:)=tr_jaja(:,:)*tr_2*iso_coeff(1)*2.0d0
   jbjb(:,:)=tr_jbjb(:,:)*tr_2*iso_coeff(2)*2.0d0
   jajb(:,:)=tr_jajb(:,:)*tr_2*iso_coeff(3)*2.0d0
   do i=1,4
      do j=1,4
          jajc(i,j)=tr_ja(i)*tr_jc(j)*iso_coeff(4)*2.0d0
          jajd(i,j)=tr_ja(i)*tr_jd(j)*iso_coeff(5)*2.0d0
          jbjc(i,j)=tr_jb(i)*tr_jc(j)*iso_coeff(6)*2.0d0
          jbjd(i,j)=tr_jb(i)*tr_jd(j)*iso_coeff(7)*2.0d0
      enddo
   enddo  
...
subroutine det_JaJc_exc(r_cc,r_cl,r_ll,r_t,r_tp)
   implicit none
   integer*4 :: i1,f1,i2,f2,i,j,fe,t1,t2
...
!!!! what we want to compute is
  ...
  ! Here we are summing over the final state isospins
   ...
   do t1=1,2
    do t2 =1,2
        iso_coeff(1) = iso_coeff(1) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
           &  *IDeltaA(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:)) !AA
        iso_coeff(2) = iso_coeff(2) + IDeltaBDag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaB(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!BB
        iso_coeff(3) = iso_coeff(3) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaB(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!AB
        iso_coeff(4) = iso_coeff(4) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaC(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!AC
        iso_coeff(5) = iso_coeff(5) + IDeltaADag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaD(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!AD
        iso_coeff(6) = iso_coeff(6) + IDeltaBDag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
           &   *IDeltaC(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!BC
        iso_coeff(7) = iso_coeff(7) + IDeltaBDag(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaD(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!BD
    enddo
   enddo 

   do i=1,7
    write(6,*)'jajac exc iso coeff(',i,') = ', iso_coeff(i)
   enddo

   jaja(:,:)=tr_jaja(:,:)*(iso_coeff(1))*2.0d0
   jbjb(:,:)=tr_jbjb(:,:)*(iso_coeff(2))*2.0d0
   jajb(:,:)=2.0d0*tr_jajb(:,:)*(iso_coeff(3))*2.0d0
   jajc(:,:)=tr_jajc(:,:)*(iso_coeff(4))*2.0d0
   jajd(:,:)=tr_jajd(:,:)*(iso_coeff(5))*2.0d0
   jbjc(:,:)=tr_jbjc(:,:)*(iso_coeff(6))*2.0d0
   jbjd(:,:)=tr_jbjd(:,:)*(iso_coeff(7))*2.0d0
 ...
 subroutine det_JpiJaJb(r_cc,r_cl,r_ll,r_t,r_tp)
   implicit none
   integer*4 :: i1,f1,i2,f2,i,j,t1,t2
   real*8 :: r_cc,r_cl,r_ll,r_t,r_tp
   ...
   do t1=1,2
    do t2 =1,2
        iso_coeff(1) = iso_coeff(1) - Iv(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaA(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:)) !piA
        iso_coeff(2) = iso_coeff(2) - Iv(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaB(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:))!piB
        enddo
   enddo 


   do i=1,2
    write(6,*)'jpijab dir iso coeff(',i,') = ', iso_coeff(i)
    enddo
  ...
  jfja=jfja*(iso_coeff(1))*2.0d0*2.0d0
  jfjb=jfjb*(iso_coeff(2))*2.0d0*2.0d0
  jsja=jsja*(iso_coeff(1))*2.0d0*2.0d0
  jsjb=jsjb*(iso_coeff(2))*2.0d0*2.0d0
  jpja=jpja*(iso_coeff(1))*2.0d0*2.0d0
  jpjb=jpjb*(iso_coeff(2))*2.0d0*2.0d0
  ...
  r_tp=-0.5d0*ci*(r_av(2,3)-r_av(3,2)) 
   ...
   do t1=1,2
    do t2 =1,2
        iso_coeff(1) = iso_coeff(1) - Iv(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaA(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:)) !piA
        iso_coeff(2) = iso_coeff(2) - Iv(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *IDeltaB(isospinor(ti1,:),isospinor(ti2,:),isospinor(t2,:),isospinor(t1,:))!piB
        enddo
   enddo 

   do i=1,2
    write(6,*)'jpijab exc iso_coeff(',i,') = ', iso_coeff(i)
   enddo
...
subroutine det_JpiJpi(r_cc,r_cl,r_ll,r_t,r_tp)
   implicit none
   integer*4 :: i1,f1,i2,f2,mu,nu,t1,t2
   real*8 :: r_cc,r_cl,r_ll,r_t,r_tp   
   ...
   do t1=1,2
    do t2 =1,2
        iso_coeff = iso_coeff - Iv(isospinor(t1,:),isospinor(t2,:), isospinor(ti1,:), isospinor(ti2,:))  &
            &  *Iv(isospinor(ti1,:),isospinor(ti2,:),isospinor(t1,:),isospinor(t2,:)) !pipi
        enddo
   enddo 



   write(6,*)'jpijpi iso coeff = ', iso_coeff

       jfjf(:,:)=tr_ff(:,:)*tr_2*(iso_coeff)*2.0d0
       do mu=1,4
          do nu=1,4
          ...
          enddo
       enddo 
       jsjs(:,:)=(jsjs(:,:)+tr_ss11(:,:)*tr_2+tr_ss22(:,:)*tr_1)*(iso_coeff)*2.0d0
       jpjp(:,:)=(jpjp(:,:)+tr_pp11(:,:)*tr_2+tr_pp22(:,:)*tr_1)*(iso_coeff)*2.0d0
       jpjs(:,:)=(jpjs(:,:)+tr_ps11(:,:)*tr_2+tr_ps22(:,:)*tr_1)*(iso_coeff)*2.0d0
       jfjs(:,:)=(tr_fs1(:,:)*tr_2*2.0d0)*(iso_coeff)*2.0d0
       jfjp(:,:)=(tr_fp1(:,:)*tr_2*2.0d0)*(iso_coeff)*2.0d0
